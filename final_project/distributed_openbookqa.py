...
# wandb.init(project="openbookQA", entity="spencerh")
...
class DataTrainingArguments:
    ...
    test_file: Optional[str] = field(
        default= None,
        metadata ={"help": "An Optional input test data file for final evaluation at the end"}
    )
...
def main():
    ...
    context_name = "fact1"
    question_header_name = "question.stem"
    ...
    if training_args.do_train:
        ...
        with training_args.main_process_first(desc="train dataset map pre-processing"):
            flatten = train_dataset.flatten()
            updated = flatten.map(choices, num_proc = data_args.preprocessing_num_workers)
            updated = updated.rename_column('answerKey', 'label')
            train_dataset = updated.map(
                preprocess_function,
                batched=True,
    ...
    if training_args.do_eval:
        ...
        with training_args.main_process_first(desc="validation dataset map pre-processing"):
            flatten = eval_dataset.flatten()
            updated = flatten.map(choices, num_proc = data_args.preprocessing_num_workers)
            updated = updated.rename_column('answerKey', 'label')
            eval_dataset = updated.map(
                preprocess_function,
                batched=True,
                num_proc=data_args.preprocessing_num_workers,
                load_from_cache_file=not data_args.overwrite_cache,
    ...
    #                 flatten = test_dataset.flatten()
    #                 updated = flatten.map(choices,  num_proc = data_args.preprocessing_num_workers)
    #                 updated = updated.rename_column('answerKey', 'label')
    #                 test_dataset = updated.map(
    #                     preprocess_function,
    #                     batched=True,
    #                     num_proc=data_args.preprocessing_num_workers,
    #                     load_from_cache_file=not data_args.overwrite_cache,
    ...
    with training_args.main_process_first(desc="saving logs"):
        with open('tmp/fact.jsonl', 'w') as f:
